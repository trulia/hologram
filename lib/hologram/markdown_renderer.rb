    ...
    def block_code(code, language)
      ...
      if language and language.include?('example')
        if language.include?('js')
          ...
          '<script>' + code + '</script>
          <div class="codeBlock jsExample"><pre>' + formatter.format(lexer.lex(code)) + '</pre></div>'
        else
          lexer = Rouge::Lexer.find(get_lexer(language))
          '<div class="codeExample">' + '<div class="exampleOutput">' + render_html(code, language) + '</div>' + '<div class="codeBlock"><pre>' + formatter.format(lexer.lex(code)) + '</pre></div>' + '</div>'
        end
      else
